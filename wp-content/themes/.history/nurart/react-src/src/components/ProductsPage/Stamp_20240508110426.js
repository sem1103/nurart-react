...
export default function Stamp() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('https://192.168.1.69/wp-json/wp/v2/shtamp_items/');
                const data = await response.json();
                let dataBox = await fetch('https://192.168.1.69/wp-json/wp/v2/media/' + data[0].acf.boxImg).then(res => res.json());
                const itemsWithImages = await Promise.all(data[0].acf.items.map(async item => {
                    const imgData = await fetch('https://192.168.1.69/wp-json/wp/v2/media/' + item.img).then(res => res.json());
                    const exampleStamb = await fetch('https://192.168.1.69/wp-json/wp/v2/media/' + item.exampleStamb).then(res => res.json());
                    item.img = imgData.source_url;
                    item.exampleStamb = exampleStamb.source_url;
                    item.boxImg = dataBox.source_url;
                    return item;
                }));
                console.log(data);
