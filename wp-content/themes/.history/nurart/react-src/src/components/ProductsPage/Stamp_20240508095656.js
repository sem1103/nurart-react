    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                let dataBox = await fetch('https://localhost/wp-json/wp/v2/media/' + data[0].acf.boxImg).then(res => res.json());
                data[0].acf.boxImg  = dataBox.source_url;
                const itemsWithImages = await Promise.all(data[0].acf.items.map(async item => {
                    ...
                    const exampleStamb = await fetch('https://localhost/wp-json/wp/v2/media/' + item.exampleStamb).then(res => res.json());
                        ...
                        items.map(item => {
                            return <div className={s.cart}>
                                ...
                                <div className={s.cart__name}>
                                    <img src="http://localhost/wp-content/uploads/2024/05/2900216-1.png" alt="hav box" style={{ opacity: item.haveBox ? 1 : 0 }} />
