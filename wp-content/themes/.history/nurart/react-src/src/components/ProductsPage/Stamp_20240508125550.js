...
export default function Stamp() {
    ...
    const searchByName = debounce((value) => {
        setLoader(true);
        setItems(prev => {
            let newItems = allItems.filter(item => item.stambId.includes(value))
            setNotFound(newItems.length ? true : false );
            setLoader(newItems.length ? false : true );
            return newItems
        });
    }, 1000);
    ...
    return (
        ...
        <>
            ...
            <div className={`container ${s.products__page}`}>
                ...
                <FilterCategory filterCategory={filterCategory} func={searchByName} placeholder='Məhsul kodu ilə axtarış...' />
                ...
                <div className={`${s.stamp__carts} ${s.carts__wrapper}`}>
                    ...
                    {
                        (!notFound && !loader) && <h3>Увы ничего не нашлось...</h3>
                    }
