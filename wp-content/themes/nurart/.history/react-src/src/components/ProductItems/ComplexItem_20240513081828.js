...
export default function ComplexItem({ productName, boxImg, haveBox, paintItem, stambSize, img, descAZ, descRu, changePillowsAz, changePillowsRu, exampleStamb, ind }) {
    ...
    return (
        <div key={productName} className={s.cart}>
            ...
            <div className={s.cart__name}>
                <img src={boxImg} className={s.have__box} alt="hav box" style={{ visibility: haveBox ? 'initial' : 'hidden' }} />

                <div>
                    <h3>{paintItem ? t('stampPaintName') : t('stambId')}: <span className={s.prod__name}>{productName}</span></h3>
                    {!paintItem && <p>{t('stambSize')}: <span className={s.prod__size}>{stambSize}</span></p>}
                </div>
            </div>

            <div className={s.product}>
                <img src={img} alt="" className={s.prod__img} />
                <div className={`${s.prod__desc} ${paintItem ? s.paint__desc : ''}`}>
                    {
                        paintItem ?
                            <div onClick={() => showMore(ind)} ref={el => pElements.current[ind] = el} dangerouslySetInnerHTML={{ __html: i18next.language == 'az' ? descAZ : descRu }} />
                            :
                            <div>
                                <h5>{t('changePillows')}</h5>
                                <ul className={s.pillows}>
                                    {
                                        i18next.language == 'az' ? changePillowsAz.map(elem => <li><span>{elem.pillow}</span></li>)
                                            :
                                            changePillowsRu.map(elem => <li><span>{elem.pillow}</span></li>)
                                    }
                                </ul>
                            </div>
                    }
                    <img src={exampleStamb} alt="" className={s.example} />
                </div>
            </div>
