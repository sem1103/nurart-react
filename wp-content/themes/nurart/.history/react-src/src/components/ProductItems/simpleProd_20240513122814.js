...
export default function SimpleProd(props) {
    ...
    return (
        <div className={`${s.cart} ${s.simpleProd}`}>
            <button className={`${s.favorite__add} ${props.favorite ? s.active : ''}`} onClick={() => !props.favorite ? addFavoriteItem({...props, prodType: true}) : removeFavoriteItem({...props, prodType: true})}>
