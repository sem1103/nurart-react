...
export default function ComplexItem(props) {
    ...
    console.log(props);
    return (
        <div key={props.productName} className={`${s.cart} ${s.stamp__carts}`}>
            <button className={s.favorite__add} onClick={() => !props.favorite ? addFavoriteItem({...props, prodType: false}) : removeFavoriteItem({...props, prodType: false})}>
