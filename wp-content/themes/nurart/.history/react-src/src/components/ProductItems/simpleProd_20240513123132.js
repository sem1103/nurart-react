...
export default function SimpleProd(props) {
    ...
    return (
        <div className={`${s.cart} ${s.simpleProd}`}>
            <button className={`${s.favorite__add} ${favorite ? s.active__favorite : ''}`} onClick={() => {
                setFavorite(!favorite)
                !props.favorite ? addFavoriteItem({...props, prodType: true}) : removeFavoriteItem({...props, prodType: true})
            }}>
