...
export default function Polygraphy() {
    ...
    const {polygraphyItems, polygraphyAllItems, fetchData, filterByName, resetItems} = useDataSlice();
    const {loader} = useLoaderSlice();

    useEffect(() => {
        resetItems();
        if(polygraphyAllItems.length) return;
      
        fetchData('http://localhost/wp-json/wp/v2/poliqrafiya_items/', 'http://localhost/wp-json/wp/v2/media/');
    }, []);
