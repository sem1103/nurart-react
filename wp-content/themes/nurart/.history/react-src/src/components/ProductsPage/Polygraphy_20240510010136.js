...
export default function Polygraphy() {
    ...
    useEffect(() => {
        resetItems();
        !allItems.polygraphy.length && fetchData('http://localhost/wp-json/wp/v2/poliqrafiya_items/', 'http://localhost/wp-json/wp/v2/media/', 'http://localhost/wp-json/acf/v2/poliqrafiya_items');        
    }, []);
                        ...
                        items.polygraphy.map(item => {
                          return  <div className={s.cart}>
                                ...
                                <div className={s.cart__desc}>
                                    ...
                                    <p>{i18next.language == 'az' ? item.productAz.desc.split('\r\n').map(line => <span>{line}<br/></span>) : item.productRu.desc.split('\r\n').map(line => <span>{line}<br/></span>)}</p>
