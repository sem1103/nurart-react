...
export default function Stamp() {
    ...
    const {shtampItems, shtampAllItems, fetchData, filterByCode, resetItems} = useDataSlice();
    const {loader} = useLoaderSlice();

    useEffect(() => {
        resetItems();
        !shtampAllItems.length && fetchData('http://localhost/wp-json/wp/v2/shtamp_items/', 'http://localhost/wp-json/wp/v2/media/');        
    }, []);
        ...
        <>
            ...
            <div className={`container ${s.products__page}`}>
                ...
                <div className={`${s.stamp__carts} ${s.carts__wrapper}`}>
                    ...
                    {
                        (!shtampItems.length && !loader) && <h3>Təəssüf ki, heç bir şey tapılmadı...</h3>
                    }
