...
export default function PlasticCards() {
    ...
    const {plasticItems, plasticAllItems, fetchData, filterByName, resetItems} = usePolygrafySlice();
    const {loader} = useLoaderSlice();

    useEffect(() => {
        resetItems();
        !plasticAllItems.length && fetchData('http://localhost/wp-json/wp/v2/plastic_cards/', 'http://localhost/wp-json/wp/v2/media/');        
    }, []);
