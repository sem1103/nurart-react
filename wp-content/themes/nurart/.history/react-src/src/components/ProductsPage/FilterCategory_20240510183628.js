        ...
        <>
            ...
            <div className={s.filter__search}>
                <div ref={filterElem} id={s.filter} className={`${openFilter ? s.open__filter : ''}`} onClick={() => setOpenFilter(!openFilter)}>
                    ...
                    <ul>
                        <li><button onClick={() => props.filterCategory('all')}>Hamısı</button></li>
                    {
                        props.filterCategories.map(item => <li><button onClick={() => props.filterCategory(item.value)}>{item.label}</button></li>)
                    }
                    </ul>
                </div>               
                    <div id={s.search} >
