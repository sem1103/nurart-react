...
export default function Polygraphy() {
    ...
    useEffect(() => {
        resetItems();
        !allItems.polygraphy.length && fetchData('http://localhost/wp-json/wp/v2/poliqrafiya_items/', 'http://localhost/wp-json/wp/v2/media/', 'http://localhost/wp-json/acf/v2/poliqrafiya_items/8');        
    }, []);
...
{
                        (!items.polygraphy.length && !loader) && <h3>Axtardım, amma tapammadım qaqa</h3>
                    }
                </div>
