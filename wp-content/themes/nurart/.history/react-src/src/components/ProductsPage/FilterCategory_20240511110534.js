        ...
        <>
            ...
            <div className={s.filter__search}>
                <div ref={filterElem} id={s.filter} className={`${openFilter ? s.open__filter : ''}`} onClick={() => setOpenFilter(!openFilter)}>
                    <p className={s.active__filter}>{t('filter')} : {!activeCategoryLabel ? t('all') : activeCategoryLabel}</p>
                    <ul>
                        <li><button onClick={() => props.filterByCategory('all', t('all'))}>{t('all')}</button></li>
                    {
                        props.filterCategories.map(item => <li><button onClick={() => props.filterByCategory(item.value, item.label)}>{activeCategoryLabel}</button></li>)
                    }
                    </ul>
                </div>               
                    <div id={s.search} >
