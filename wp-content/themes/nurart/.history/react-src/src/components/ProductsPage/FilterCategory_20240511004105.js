...
export default function FilterCategory(props) {
    ...
    const {searchInput} = useDataSlice();
        ...
        <>
            ...
            <div className={s.filter__search}>
                <div ref={filterElem} id={s.filter} className={`${openFilter ? s.open__filter : ''}`} onClick={() => setOpenFilter(!openFilter)}>
                    ...
                    <ul>
                        <li><button onClick={() => props.filterByCategory('all')}>Hamısı</button></li>
                    {
                        props.filterCategories.map(item => <li><button onClick={() => props.filterByCategory(item.value)}>{item.label}</button></li>)
                    }
                    </ul>
                </div>               
                    <div id={s.search} >
                    <input type="text" placeholder={props.placeholder} value={searchInput} onChange={(e)=> props.filterByName(e.target.value)}/>
                    </div>
            </div>
