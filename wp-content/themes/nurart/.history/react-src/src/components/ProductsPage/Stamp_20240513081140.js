...

                    {
                        loader ? <Loader />
                            ...
                            items.shtamp.map((item, ind) => {
                                return <ComplexItem productName={item.productName} boxImg={item.boxImg}  haveBox={item.haveBox} paintItem={item.paintItem} stambSize={item.stambSize} img={item.img} descAZ={item.descAZ} descRu={item.descRu} changePillowsAz={item.changePillowsAz} changePillowsRu={item.changePillowsRu} exampleStamb={item.exampleStamb}/>
                            })
                    }
