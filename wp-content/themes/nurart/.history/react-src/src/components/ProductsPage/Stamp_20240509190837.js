...
export default function Stamp() {
    ...
    const {stampItems, stampAllItems, fetchData, filterByCode, resetItems} = useDataSlice();
    const {loader} = useLoaderSlice();

    useEffect(() => {
        resetItems();
        !stampAllItems.length && fetchData('http://localhost/wp-json/wp/v2/poliqrafiya_items/', 'http://localhost/wp-json/wp/v2/media/');        
    }, []);
        ...
        <>
            ...
            <div className={`container ${s.products__page}`}>
                ...
                <div className={`${s.stamp__carts} ${s.carts__wrapper}`}>
                    ...
                    {
                        (!stampItems.length && !loader) && <h3>Təəssüf ki, heç bir şey tapılmadı...</h3>
                    }
