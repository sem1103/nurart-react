            ...
            fetchData: async (mainUrl, imgUrl) => {
                try {
                    ...
                    const items = await Promise.all(data[0].acf.items.map(async item => {
                        ...
                        item.exampleStamb = item.exampleStamb = data[0].acf.boxImg && await fetch(imgUrl + item.exampleStamb).then(res => res.json()).then(res => res.source_url);
                        item.boxImg = item.boxImg && await fetch(imgUrl + item.boxImg).then(res => res.json()).then(res => res.source_url);
                        return item;
                    }));
                    console.log(data[0].acf.items);
                    switch (true) {
