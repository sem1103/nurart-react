    ...
    devtools(
        (set,get) => ({
            ...
            addFavoriteItem: (item) =>{
                ...
                localStorage.setItem('favorites', JSON.stringify([...item, item]));
            },
            getFavoriteItems: () => {
                Object.keys(get().allItems).forEach(item => item.forEach(elem => {
                    let newFavorite = elem.img == localStorage[elem.img] && localStorage.getItem(elem.img)
                    set(state => ({favoriteItems: [state.favoriteItems, newFavorite]}))
                }))
            }
        })
    )
